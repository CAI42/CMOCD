...
from ultralytics.nn.modules import (
    ...
    VTFCF,
    VTFFA, VTFFA1, VTFFA2,
    CFFA_STN, CFFA_PerspectiveSTN,
    VTFF_gate,PerspectiveSTN_DDF,
    PerspectiveSTN_gateDDF,STN_DDF,STN_gateDDF,
)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules :
            ...
            c2 = ch[f[0]]
            args = [c2]
        elif m is EfficientCoarseFeatureTransform:
            c2 = ch[f[0]]
            args = [c2]
        elif m is CoarseMatching:
            c2 = ch[f[0]]
            args = [c2]
        elif m is VTFCF:
            c2 = ch[f[0]]
            ...
            c2 = ch[f[0]]
            args = [c2]
        elif m is AdaptiveCoarseFeatureTransform:
            c2 = ch[f[0]]
            args = [c2]
        elif m is VTFF_gate:
            c2 = ch[f[0]]
            ...
            c2 = ch[f[0]]
            args = [c2]
        elif m is CFFA_PerspectiveSTN:
            c2 = ch[f[0]]
            args = [c2]
        elif m is STN_DDF:
            c2 = ch[f[0]]
            args = [c2]
        elif m is STN_gateDDF:
            c2 = ch[f[0]]
            args = [c2]
        elif m is PerspectiveSTN_DDF:
            c2 = ch[f[0]]
            args = [c2]
        elif m is PerspectiveSTN_gateDDF:
            c2 = ch[f[0]]
            args = [c2]
        elif m is Add2:
            c2 = ch[f[0]]
        ...
        try:
            ...
            print(f"实例化模块 {m} 时出错: {e}")
            print(f"参数: {args}")
            raise

        if verbose and i == 0:
            setattr(m_, 'print_shape', True)
        else:
            setattr(m_, 'print_shape', False)
