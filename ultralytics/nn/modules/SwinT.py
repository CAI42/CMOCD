...
import math
from .spatial_transformer import CEM1
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size1, window_size2, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.window_size2 = window_size2  # Wh, Ww
    ...
    def forward(self, x, y, mask=None):
        ...
        kv = self.qkv(y).reshape(B_, N, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)#[B, num_head, N, Channel]
        ...
        #     self.window_size1[0] * self.window_size1[1], self.window_size2[0] * self.window_size2[1], -1)  # Wh*Ww,Wh*Ww,nH
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, y, semantic=None):
        # B HW C is the shape of x, which has been flattened and transformed
        ...
        # partition windows
        x_windows = window_partition(shifted_x, self.window_size1)  # [nWindow*B, window_size, window_size, C]
        x_windows = x_windows.view(-1, self.window_size1 * self.window_size1, C)  # [nWindow*B, window_size*window_size, C]
        #y_window
        y_windows = window_partition(shifted_y, self.window_size2)  # [nWindow*B, window_size, window_size, C]
        y_windows = y_windows.view(-1, self.window_size2 * self.window_size2,
                                   C)  # [nWindow*B, window_size*window_size, C]
        if semantic is not None:
            ...
            sem_windows = window_partition(shifted_sem, self.window_size2)  # [nWindow*B, window_size, window_size, C]
            sem_windows = sem_windows.view(-1, self.window_size2 * self.window_size2, C)
            y_windows = y_windows * sem_windows
        # W-MSA/SW-MSA
        ...
        shifted_x = window_reverse(attn_windows, self.window_size1, H, W)  # B H' W' C
...
class SwinT(nn.Module):
    """ 采用 SwinTransformerBlock 的 GPT 类 """
    ...
    def __init__(self, d_model, h=8, block_exp=4,
        ...
        # SwinTransformerBlock 替换原来的 TransformerBlock
        self.swin_blocks = nn.Sequential(*[self._build_swin_block(d_model, h, block_exp, attn_pdrop, resid_pdrop)
    ...
    def _build_swin_block(self, d_model, h, block_exp, attn_pdrop, resid_pdrop):
        # 构建 SwinTransformerBlock
        return SwinTransformerBlock(
            dim=d_model,
            input_resolution=(self.vert_anchors, self.horz_anchors),
            num_heads=h,
            window_size1=4,  # 示例窗口大小
            window_size2=8,
            shift_size=0,
            up_ratio=1,
    ...
    def forward(self, x):
        """
        Args:
            x (tuple): 包含两个元素的元组，分别是 RGB 图像特征和红外图像特征
        """
        rgb = x[0]
        ir = x[1]
        ...
        x = x.view(bs, 2, self.vert_anchors * self.horz_anchors, self.n_embd)  # 重塑为 (B, 2, H*W, C)
        rgb_x, ir_x = x.unbind(dim=1)  # 分离 RGB 和红外特征

        for block in self.swin_blocks:
            rgb_x = block(rgb_x, ir_x)  # 使用 SwinTransformerBlock 处理特征
            ir_x = block(ir_x, rgb_x)   # 可以选择对红外特征也进行处理，或根据需要调整

        x = torch.cat([rgb_x, ir_x], dim=1).view(bs, -1, self.n_embd)  # 合并特征
