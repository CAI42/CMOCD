        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                ...
                if self.use_simotm ==  'Gray2BGR':
                    ...
                    # 合并成四通道图像
                    im_infrared_channel = im_visible if len(im_infrared.shape) == 2 else im_infrared[:, :, 0]
                    im = cv2.merge((b, g, r, im_infrared_channel))
                    ...
                    # 以热红外大小为基准图像大小
                    target_height, target_width = b2.shape[:2]

                    # 调整其他通道的尺寸
                    b = cv2.resize(b, (target_width, target_height))
                    g = cv2.resize(g, (target_width, target_height))
                    r = cv2.resize(r, (target_width, target_height))
